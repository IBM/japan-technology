...
## まとめ
この演習では、OpenAPI形式で気象情報のAPIを取り込み、AIエージェントのツールとして利用できるようにしました。
気象情報から得られるWMO気象コードを使って、天気の説明を取得するために、CSV形式のファイルを使って、知識を取り込みました。
都市名から位置情報への変換は、Nominatim APIではなく、基盤モデルの学習済み知識を用いているので、呼び出しの制約がなく、利用しやすいエージェントになりました。
