...
- ✅ Planモードを使用してバグを特定し、修正計画を立てる
- ✅ SQLインジェクションの脆弱性を認識する
- ✅ XSS攻撃ベクトルを特定する
- ✅ ハードコードされた秘密情報と認証情報を見つける
- ✅ Agentモードを使用してセキュリティ修正を実装する
- ✅ 安全なコーディングのベストプラクティスを適用する

## 前提条件

始める前に、以下のものを用意してください:
- [ ] lab1を完了していること（またはFlaskとJavaScriptに精通していること）
...
```
vulnerable-app/backend/ のコードを分析し、次の点を説明してください。
...
```
vulnerable-app/frontend/ にあるフロントエンドコードを分析し、以下の点について説明してください。
...
## Step 2: Planモードによるバグの特定（10分）

それでは、Planモードを使用して、すべての問題を体系的に特定していきましょう。

### 2.1: Planモードに切り替える

**Ask モード** から **Plan モード** に変更します。

### 2.2: セキュリティ分析の依頼

> **💡 Bob Findingsの活用事例**
> Bob Findings は、セキュリティの脆弱性、コード品質の問題、コンプライアンス違反を自動的にスキャンできます。これからリクエストする分析は、Bob のセキュリティ脆弱性検出機能を示すもので、基本的な静的分析を超えて、状況に応じた推奨事項を提供します。

**Bobへのプロンプト:**

```
vulnerable-app/ のコードベースを分析し、セキュリティ脆弱性を特定してください。
...
この手順では、各脆弱性の種類について詳しく説明します。これらのセキュリティ概念を既に理解している場合は、[Step 4：Agentモードを使用した修正の実装](#step-4-agentモードを使用した修正の実装10分)に進んでください。
...
## Step 4: Agentモードを使用した修正の実装（10分）

それでは、BobのAgentモードを使ってすべての脆弱性を修正しましょう。
...
**重要**: Bobは `vulnerable-app/` 内のファイルに直接修正を加えました（別のソリューションフォルダではなく）。脆弱なコードは安全なコードに置き換えられています。
...
**変更前と変更後を比較する:**
元の脆弱なコードを確認したい場合は、修正のリファレンス実装が含まれている `solution/` ディレクトリを確認してください。
...
- ✅ セキュリティ分析にはPlanモードを使用する
- ✅ SQLインジェクションの脆弱性を特定する
- ✅ XSS攻撃ベクトルを認識する
- ✅ ハードコードされた秘密情報を見つけて修正する
- ✅ 安全なコーディング手法を導入する
- ✅ Agentモードを使用してセキュリティ問題を修正します

> **🎯 Bob Findingsの活用事例**
> このラボでは、Bobの自動セキュリティ分析機能を体験しました。Bob Findings は、コードの脆弱性を継続的に監視し、実行可能な修正ガイダンスを提供します。このプロアクティブなアプローチにより、セキュリティ問題が本番環境に及ぶ前に発見できるため、リスクと技術的負債を軽減できます。
...
**次のチャレンジに挑戦する準備はできましたか？** → [lab3 開始: Code Translation](../lab3/README.md)
