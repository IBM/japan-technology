...
<body>
        ...
        <div class="dashboard-grid">
            ...
            <div class="card customer-selector">
                ...
                <div id="transferForm" style="display: none; margin-top: 20px;">
                    <h3>Transfer to Savings Bank</h3>
                    <p style="color: #666; margin-bottom: 15px;">Transfer funds from your Investment account to your Savings account</p>
                    <div class="form-group">
                        <label>Amount ($)</label>
                        <input type="number" id="transferAmount" step="0.01" placeholder="Enter amount to transfer">
                    </div>
                    <button class="operation-btn" onclick="executeTransfer()">Transfer to Savings Bank</button>
                ...
                <div id="investForm" style="display: none; margin-top: 20px;">
                    ...
                    <div class="form-group">
                        <label>Amount ($)</label>
                        <input type="number" id="investAmount" step="0.01" placeholder="Enter investment amount">
                    </div>
                    <div class="form-group">
                    ...
                    <button class="operation-btn" onclick="executeInvestment()">Execute Investment</button>
                    <button class="operation-btn" onclick="hideInvestForm()" style="background: #6c757d;">Cancel</button>
                </div>
                ...
                <div id="customQueryForm" style="display: none; margin-top: 20px;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="queryDescription" rows="3" placeholder="e.g., 'investments over $1000'"></textarea>
                    </div>
    ...
    <script>
                    ...
                    if (result.result_data.pie_chart) {
                        resultContainer.innerHTML = `
                            <div class="success-message">
                                <strong>Investment Analytics Generated Successfully</strong><br>
                                Total Invested: $${result.result_data.total_invested || 0}
                            </div>
                        `;
        ...
        async function executeTransfer() {
            ...
            const resultContainer = document.getElementById('resultContainer');
            resultContainer.innerHTML = '<div class="loading">Processing transfer to Savings Bank...</div>';
                    ...
                    resultContainer.innerHTML = `
                        <div class="success-message">
                            ...
                            <div style="font-size: 1.1em;">
                                <strong>Amount Transferred:</strong> $${amount.toFixed(2)}<br>
                                <strong>From:</strong> Investment Bank<br>
                                <strong>To:</strong> Savings Bank<br><br>
                                <strong>New Investment Balance:</strong> $${result.new_balance.toFixed(2)}
                            </div>
                        </div>
                    `;
                    hideTransferForm();
        ...
        async function executeInvestment() {
            ...
            const resultContainer = document.getElementById('resultContainer');
            resultContainer.innerHTML = '<div class="loading">Processing investment...</div>';
                ...
                if (result.success) {
                    resultContainer.innerHTML = `
                        <div class="success-message">
                            ...
                            Amount: $${amount.toFixed(2)} in ${category}<br>
                            New Portfolio Value: $${result.result_data.new_balance.toFixed(2)}
                        </div>
                    `;
                    hideInvestForm();
                    loadCustomers(); // Refresh customer balances
                } else {
                    resultContainer.innerHTML = `<div class="error-message">Investment Failed: ${result.error}</div>`;
                }
...
def transfer_to_savings():
    ...
    try:
        ...
        if current_balance < amount:
            return jsonify({'success': False, 'error': f'Insufficient funds. Current balance: ${current_balance:.2f}'}), 400
        ...
        if error:
            return jsonify({'success': False, 'error': 'Failed to deduct from account'}), 500
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return jsonify({
                    'success': True,
                    'message': f'Successfully transferred ${amount:.2f} to Savings Bank',
                    'amount': amount,
                    'new_balance': new_balance
                })
            else:
                ...
                return jsonify({
                    'success': False,
                    'error': f'Transfer to Savings Bank failed: {response.text}'
                }), 500
            ...
            return jsonify({
                'success': False,
                'error': f'Failed to connect to Savings Bank: {str(e)}'
            }), 500
