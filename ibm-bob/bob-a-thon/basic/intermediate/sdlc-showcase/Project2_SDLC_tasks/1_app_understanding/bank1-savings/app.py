...
def add_dev_cors_headers(response):
    """Allow local frontend dev servers to call the API during hands-on verification."""
...
BANK2_URL = os.getenv('BANK2_URL', 'http://localhost:5000')
...
MESSAGE_NO_SPENDING_DATA = "支出データがありません"
MESSAGE_COULD_NOT_VERIFY_BALANCE = "残高を確認できませんでした"
...
def generate_spending_analytics(customer_id: int) -> Dict:
    ...
    if not result:
        return {'message': MESSAGE_NO_SPENDING_DATA}
...
def receive_transfer():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'Investment Bank からの振替を受け付けました: ${amount:.2f}',
...
def transfer_funds():
    ...
    if error or not balance_result:
        return jsonify(build_response('Unknown', 'transfer', None, False,
                                     MESSAGE_COULD_NOT_VERIFY_BALANCE)), 500
    ...
    if current_balance < amount:
        return jsonify(build_response(customer_name, 'transfer', None, False,
                                     f'残高不足です。現在の残高: ${current_balance:.2f}')), 400
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify(build_response(customer_name, 'transfer', None, False,
                                         'Investment Bank への振替に失敗したため、取引をロールバックしました')), 500
        ...
        return jsonify(build_response(customer_name, 'transfer', None, False,
                                     f'Investment Bank に接続できなかったため、取引をロールバックしました: {str(e)}')), 500
