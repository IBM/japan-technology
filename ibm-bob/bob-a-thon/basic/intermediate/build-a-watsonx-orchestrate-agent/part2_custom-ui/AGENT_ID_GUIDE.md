...
4. **Agent IDを確認**
   ...
   ```
   https://{YOUR_REGION}.dl.watson-orchestrate.ibm.com/build/agent/edit/{AGENT_ID}
   ```

5. **API URLを構築**
   - 手順4で確認した `{YOUR_REGION}` と `{AGENT_ID}` を使って以下のURLを組み立てます：
   ```
   https://api.{YOUR_REGION}.dl.watson-orchestrate.ibm.com/instances/{YOUR_INSTANCE_ID}/v1/agents/{AGENT_ID}/generation_stream
   ```
...
```env
AGENT_URL = "https://api.{YOUR_REGION}.dl.watson-orchestrate.ibm.com/instances/{YOUR_INSTANCE_ID}"
```
...
```env
AGENT_URL = "https://api.{YOUR_REGION}.dl.watson-orchestrate.ibm.com/instances/{YOUR_INSTANCE_ID}/v1/agents/{YOUR_AGENT_ID}/generation_stream"
```
