...
**アクション4.1:** 新しいタスクを開始し、**Agent**モードで以下のプロンプトを入力します:
...
- watsonx.ai API URLを設定します（東京リージョンは`https://jp-tok.ml.cloud.ibm.com`）
- 注意: LLM名として`meta-llama/llama-3-3-70b-instruct`を使用してください。Bobは利用できなくなった別のモデルを使用する可能性があります。watsonx.aiインスタンスを確認し、利用可能なほかのLLMを選択することもできます。
...
- .gitignore はBob自身のシステムポリシーでブロックされています。代わりに gitignore.txt として作成されます。
![gitignore example](screenshots/gitignore_example.png)
- `gitignore.txt`ファイルを`.gitignore`にリネームします
...
```
![主要なパッケージ](screenshots/package_list.png)
...
メール確認が出る場合がありますが、無視してEnterを押してください。

**✓ チェックポイント:** http://localhost:8501/ でアプリのUIが表示されるはずです。
ローカルホストでの起動に若干タイムラグがありますが、表示するまで数秒〜数十秒かかる場合があります。
...
エラーがターミナルにも表示されます。エラーの詳細をBobと共有し、問題の修正を依頼します。

**アクション8.3:** **Agent**モードで新しいタスクを作成します。ターミナルでエラーを選択し、右クリックしてコピーし、Bobとのチャットに貼り付けます。**Send**をクリックしてエラーの詳細をBobと共有します。

![エラーの修正](screenshots/share_error_from_terminal.png)

また、ブラウザーでエラーメッセージを全部選択し、右クリックしてコピーし、Bobとのチャットに貼り付けてもいいです。
![エラーの修正](screenshots/share_error_from_browser.png)
